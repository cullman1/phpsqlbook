...
class CategoryManager
{
  ...
  public function update($category){
    $pdo = $this->pdo;
    $sql = 'UPDATE category SET name = :name, description = :description, navigation = :navigation WHERE id = :id';//SQL
    $statement = $pdo->prepare($sql);                                  // Prepare
    $statement->bindParam(':id', $category->id, PDO::PARAM_INT);       // Bind id
    $statement->bindParam(':name', $category->name);                   // Bind name
    $statement->bindParam(':description', $category->description);     // Bind description
    $statement->bindParam(':navigation', $category->navigation);       // Bind navigation
    try {
      $statement->execute();
      $result = TRUE;
    } catch (PDOException $error) {                                    // Otherwise
