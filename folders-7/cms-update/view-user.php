...
if (isset($_GET['name'])) {  // If check passes
  $user       = $cms->userManager->getUserBySeoName($_GET['name']);
  if (empty($user) ) {
  CMS::redirect('page-not-found.php');
  } 
  $my_profile = $cms->userManager->isCurrentUser($user->seo_name);
}
...
$users_articles   = $cms->articleManager->getArticleSummariesByUserId($user->user_id);
