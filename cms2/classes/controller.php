...
public function assemblePage($part, $content, $contenttype) {
  $this->registry->set('LayoutTemplate',
   new LayoutTemplate($this->controller, $this->action, 
   $this->parameters, $this->pdo ));  
  ...
  default:
    ...
    foreach ($content as $content_part) {
      if ($content_part == "content") {
        ...
        if ($this->parameters[0]==""||is_numeric($this-> 
         parameters[0])|| isset($_REQUEST["search"])) {
          $lt->getPart($content_part, $article_ids[$i]);
        } else {  
          $result2 = $dbh->getArticleByName($this->pdo,         
           $this->parameters); 
