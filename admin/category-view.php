...
$tsql = "select article_id, title, content, category_name, category.category_id, user_name, date_posted, role_id FROM article JOIN user ON article.user_id = user.user_id JOIN category ON article.category_id = category.category_id where category.category_id = ".$_REQUEST["categoryid"]." order by article_id";
...
/* Query SQL Server for total records data. */
$tsql3 = "select category_name from category where category_id= ".$_REQUEST["categoryid"];
