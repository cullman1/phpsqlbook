...
require('../classes/db-handler.php');
require('../classes/layout-template.php');
...
public function createPageStructure() {     
 switch($this->controller) {
  case "article": 
   $this->page_html = array("header","search", "menu", 
    "article","footer");
   $this->content_html = array("content","like","comments");
   break;
  case "admin":
   $this->page_html = array("header", "menu","article",
    "footer");
...
if ($this->parameters[0]==""||is_numeric($this->parameters[0])|| isset($_REQUEST["search"])) {
 $lt->getPart($content_part, $article_ids[$i]);
  ...
  $recordset2 = $dbh->getArticleByName($this->pdo,$this->parameters); 
