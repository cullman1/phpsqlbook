 ...
 <div id="category_container" class="<?php echo $row['category_template']; ?>">
      ...
      $select_medialink_sql = "select * from media_link join media on media.media_id = media_link.media_id WHERE media.article_id = ".$row['article_id'];
        ...
        <?php 
          ...
          $select_comments_sql = "select comments_id, comment_repliedto_id, comment, full_name, comment_date FROM comments JOIN user ON comments.user_id = user.user_id WHERE article_id = ".$row['article_id']." Order by Comments_id asc";
