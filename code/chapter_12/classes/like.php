...
class Like {
    ...
    function setLiked() {
        $query = "SELECT count(*) AS liked FROM liked WHERE
    ...
    function setTotal() {
        $query = "SELECT count(*) AS total 
              FROM liked WHERE article_id=:articleid"; 
        $statement = $this->connection->prepare($query);
        $statement->bindParam(':articleid', $this->articleId);
        $statement->execute();
        $statement->setFetchMode(PDO::FETCH_OBJ);  
    ...
    function add() { 
        $sql = "INSERT INTO liked (user_id, article_id)  
    ...
    function delete() {
        $sql = "DELETE FROM liked WHERE user_id= :userid 
