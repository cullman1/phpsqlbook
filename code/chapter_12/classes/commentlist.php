...
class CommentList {
  ...
  function __construct($comment_list) {
    ...
    foreach($comment_list as $row) {
      $comment = new Comment($row->{"comments.id"},$row->{"comments.article_id"}, $row->{"user.forename"} . " ". $row->{"user.surname"},$row->{"comments.comment"} , $row->{"comments.posted"} , $row->{"comments.repliedto_id"}  );
