...
function check_article_id($dbHost, $id, $sql) {
$statement = $dbHost->prepare($sql);
$statement->bindParam(":id",$id);
$select_count_rows = $statement->fetchColumn();
$num_rows = count($select_count_rows);
return $num_rows;
}
$num_rows = check_article_id($dbHost, $_GET["article_id"], $sql);
if($num_rows==0){
    echo "<div class='box2'>No article of that id has been published.</div>";
} else {
  $statement = get_article_by_id($dbHost, $_GET["article_id"], $sql);
} ?>
<div style="margin-top:150px;"></div>
<?php while($row = $statement->fetch()) {  ?>
<div style="width:500px; margin-left:20px;" class="<?php echo htmlspecialchars($row['category_template']); ?>">
