<?php require_once('../includes/db_config.php');
      if (!empty($_POST['pwd']) && !empty($_POST['firstName']) && !empty($_POST['lastName']) 
          ...
          if(count($sel_user_rows)>0) {
              echo ("<div class='wholeform'>User already exists</div>");        
          }	
          else {
              $ins_user_set = $dbHost->prepare("INSERT INTO user (full_name, password, email, role_id, date_joined, active) VALUES 
              (:name,:password,:email,:date,2,0)");
          ...
          echo ("<div class='wholeform'>Not all fields were completed</div>");    
      } ?>