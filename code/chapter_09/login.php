...
function get_user_by_email($email, $password) {
    $query = "SELECT id, forename, surname, email, image FROM user  WHERE email =:email AND password= :password";
...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($validation_failed == true) {
        ...
        if ($user) {
            create_session($user);
            header('Location: login-home.php'); // do include on admin pages
        } else {
