...
function validate_form($password, $confirm) {
    $valid = (filter_var($password, FILTER_VALIDATE_REGEXP,             array('options' => array('regexp'=>'(?=^.{8,}$)((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$' ))) ? '' : 'Your password is not strong enough');
...
<form method="post" action="reset-password.php">
  ...
  <label>Confirm Your Password: <input type="password" name="confirm" /></label>  <br/><br/>
